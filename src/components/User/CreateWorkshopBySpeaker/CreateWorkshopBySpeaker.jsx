...
import { getWorkshopRequest } from "../../../redux/actions/request.actions";
import WorkshopStatus from "./WorkshopStatus/WorkshopStatus";

export const CreateWorkshopBySpeaker = () => {
  ...
  const workshopRequest = request?.find((req) => req.workshopRequest?.status);

  useEffect(() => {
    dispatch(getWorkshopRequest());
  }, []);
  ...
  const handleCreateWorkshop = async () => {
    ...
    dispatch(createWorkshopBySpeaker(workshop))
      .unwrap()
      .then(() => {
        autoCloseAlert("Se ha enviado una solicitud de Workshop", "success");
        resetTitle();
        resetDescription();
        resetNewSpeaker();
        resetNewAttendee();
  ...
  return (
    <>
      {isLoading && <Loader />}
      {workshopRequest?.workshopRequest?.status === "RECHAZADA" ||
      workshopRequest?.workshopRequest?.status === "PENDIENTE" ? (
        <WorkshopStatus request={workshopRequest} />
      ) : (
        <>
          ...
          <Grid container spacing={2} sx={{ p: 2 }}>
            ...
            <Grid item xs={12} md={6}>
              ...
              <FormControl fullWidth sx={{ my: 2 }}>
                <ValidatedTextField
                  ...
                  helperText={
                    descriptionError ? "La descripción no es válida" : ""
                  }
                  label="Descripción"
                  type="text"
                  multiline
            ...
            <Grid item xs={12} md={6}>
              <Box
                sx={{ display: "flex", gap: 2, minHeight: { xs: 50, md: 100 } }}
              >
                <ValidatedTextField
                  value={newSpeaker}
              ...
              >
                <ValidatedTextField
                  ...
                  helperText={
                    newAttendeeError
                      ? "El nombre del asistente no es válido"
                      : ""
                  }
                  label="Asistente"
                  type="text"
